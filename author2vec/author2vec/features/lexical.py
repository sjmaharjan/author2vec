...
__all__ = ["NGramTfidfVectorizer", "CategoricalCharNgramsVectorizer"]
...
class CategoricalCharNgramsVectorizer(TfidfVectorizer):
    ...
    _whole_word = (
        lambda self, x, y, i, n: not (re.findall(r"(?:\W|\s)", x))
        and (i == 0 or y[i - 1] in self._slash_W)
        and (i + n == len(y) or y[i + n] in self._slash_W)
    )
    _mid_word = lambda self, x, y, i, n: not (
        re.findall(r"(?:\W|\s)", x)
        or i == 0
        or y[i - 1] in self._slash_W
        or i + n == len(y)
        or y[i + n] in self._slash_W
    )
    ...
    _prefix = (
        lambda self, x, y, i, n: not (re.findall(r"(?:\W|\s)", x))
        and (i == 0 or y[i - 1] in self._slash_W)
        and (not (i + n == len(y) or y[i + n] in self._slash_W))
    )
    _suffix = (
        lambda self, x, y, i, n: not (re.findall(r"(?:\W|\s)", x))
        and (not (i == 0 or y[i - 1] in self._slash_W))
        and (i + n == len(y) or y[i + n] in self._slash_W)
    )
    ...
    def __init__(
        ...
        super(CategoricalCharNgramsVectorizer, self).__init__(
            input=input,
            encoding=encoding,
