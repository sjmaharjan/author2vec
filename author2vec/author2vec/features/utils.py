...
def fetch_features_vectorized(data_dir, features, corpus):
    def extract_feature(feature):
        ...
        if os.path.exists(target_file):
            ...
            joblib.dump(
                (
                    [book.book_id for book in corpus.X_train],
                    [book.book_id for book in corpus.X_test],
                ),
                target_index_file,
            )
...
def test_fetch_features_vectorizer(
    data_dir, features=["writing_density", "readability"]
):
    X_train, Y_train, _, _ = fetch_features_vectorized(data_dir, features, "")
    X_train_w, Y_train_w, _, _ = fetch_features_vectorized(
        data_dir, "writing_density", None
    )
    X_train_r, Y_train_r, _, _ = fetch_features_vectorized(
        data_dir, "readability", None
    )
...
def johns_features():
    ...
    X_train_rnn = []
    X_test_rnn = []
    X_train, X_test = joblib.load(
        "/home/sjmaharjan/Books/booksuccess/vectors/unigram_index.pkl"
    )
    ...
    joblib.dump(
        (
            np.array([target[fnames.index(fname)] for fname in X_train]),
            np.array([target[fnames.index(fname)] for fname in X_test]),
        ),
        "rnn_labels.pk",
    )
